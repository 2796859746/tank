...
import cn.hzw.game.business.*
import cn.hzw.game.enums.Direction
import cn.hzw.game.model.*
...
//    private val views = arrayListOf<View>()
    ...
    override fun onCreate() {
        ...
        lines.forEach{line ->
            ...
            line.toCharArray().forEach { column ->
                when(column){
                    '砖' -> views.add(Wall(Config.block * columnNum,Config.block * lineNum))
                    '铁' -> views.add(Steel(Config.block * columnNum,Config.block * lineNum))
                    '草' -> views.add(Grass(Config.block * columnNum,Config.block * lineNum))
                    '水' -> views.add(Water(Config.block * columnNum,Config.block * lineNum))
                    '敌' -> enemyBornLocation.add(Pair(Config.block * columnNum,Config.block * lineNum))
                }
                columnNum ++
            }
            lineNum ++
    ...
    override fun onRefresh() {
        ...
        views.filter { it is Movable }.forEach{ move ->
            ...
            //不要和自己比较
            //2)找到阻塞的物体
            views.filter { (it is Blockable) and (move != it) }.forEach blogTag@{ block ->
                //3)遍历集合，找到是否发生碰撞
                //move和block是否碰撞
                block as Blockable
                //获得碰撞的方向
